...
import io.choerodon.issue.app.assembler.StateMachineNodeAssembler;
...
import io.choerodon.issue.infra.annotation.ChangeStateMachineStatus;
...
public class StateMachineTransformServiceImpl extends BaseServiceImpl<StateMachineTransformDraft> implements StateMachineTransformService {
    ...
    @Override
    public Map<Long, Map<Long, List<TransformDTO>>> queryStatusTransformsMap(Long organizationId, List<Long> stateMachineIds) {
        if (stateMachineIds == null || stateMachineIds.isEmpty()) {
            return null;
        }
        Map<Long, Map<Long, List<TransformDTO>>> resultMap = new HashMap<>(stateMachineIds.size());
        List<StateMachineTransform> allTransforms = transformDeployMapper.queryByStateMachineIds(organizationId, stateMachineIds);
        List<TransformDTO> allTransformDtos = modelMapper.map(allTransforms, new TypeToken<List<StateMachineTransform>>() {
        }.getType());
        Map<Long, List<TransformDTO>> transformStateMachineIdMap = allTransformDtos.stream().collect(Collectors.groupingBy(TransformDTO::getStateMachineId));
        ...
        for (Long stateMachineId : stateMachineIds) {
            List<TransformDTO> transforms = transformStateMachineIdMap.get(stateMachineId) != null ? transformStateMachineIdMap.get(stateMachineId) : new ArrayList<>();
            List<TransformDTO> typeAll = transforms.stream().filter(x -> x.getType().equals(TransformType.ALL)).collect(Collectors.toList());
            Map<Long, List<TransformDTO>> startListMap = transforms.stream().collect(Collectors.groupingBy(TransformDTO::getStartNodeId));
            ...
            Map<Long, List<TransformDTO>> statusMap = new HashMap<>(nodes.size());
            for (StateMachineNode node : nodes) {
                List<TransformDTO> nodeTransforms = startListMap.get(node.getId()) != null ? startListMap.get(node.getId()) : new ArrayList<>();
