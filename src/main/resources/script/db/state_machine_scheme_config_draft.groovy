...
databaseChangeLog(logicalFilePath: 'state_machine_scheme_config_draft.groovy') {
    changeSet(id: '2018-11-19-add-table-state-machine-scheme-config-draft', author: 'shinan.chenX@gmail') {
        createTable(tableName: "state_machine_scheme_config_draft") {
        ...
        createIndex(tableName: "state_machine_scheme_config_draft", indexName: "state_machine_scheme_config_draft_n1") {
            column(name: "scheme_id", type: "BIGINT UNSIGNED")
        }
        createIndex(tableName: "state_machine_scheme_config_draft", indexName: "state_machine_scheme_config_draft_n2") {
            column(name: "issue_type_id", type: "BIGINT UNSIGNED")
        }
        createIndex(tableName: "state_machine_scheme_config_draft", indexName: "state_machine_scheme_config_draft_n3") {
    ...
    changeSet(id: '2018-11-23-fix-table-state-machine-scheme-config-draft', author: 'shinan.chenX@gmail') {
        sql(stripComments: true, splitStatements: false, endDelimiter: ';') {
            "insert into state_machine_scheme_config_draft select * from state_machine_scheme_config;"
        }
    }
}