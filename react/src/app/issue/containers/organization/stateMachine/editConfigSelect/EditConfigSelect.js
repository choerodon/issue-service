...
class EditConfigSelect extends Component {
  ...
  handleCancel = () => {
    ...
    history.push(`/issue/state-machines/${machineId}/editconfig/${configId}?type=organization&id=${id}&name=${encodeURIComponent(name)}&organizationId=${organizationId}`);
  }
    ...
    return (
      <Page>
        <Header
          ...
          backPath={`/issue/state-machines/${machineId}/editconfig/${configId}?type=${type}&id=${projectId}&name=${encodeURIComponent(name)}&organizationId=${orgId}`}
        >
          <Button
